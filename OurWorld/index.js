/**
 * rpBot by Avengineering and Amp!
	...
	try {
		...
		await Coglib.ReplyOrEditReply(interaction, {
			content:
				...
				`reach out to my supervisor @avengineering via Discord to submit a bug ` +
				`report.`,
			embeds: [],
			components: [],
			ephemeral: true
...
async function initialize() {
	client.login(token).catch((e) => {
		Winchalk.error(`There was a problem initializing rpBot.`);
	})
}
